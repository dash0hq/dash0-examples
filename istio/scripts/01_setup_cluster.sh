...
# Setup Kind cluster for Istio demo
set -e
...
echo -e "${GREEN}🚀 Setting up Kind cluster for Istio demo${NC}"
echo "================================="
...
if kind get clusters 2>/dev/null | grep -q "istio-demo"; then
    echo -e "${YELLOW}⚠️  Cluster 'istio-demo' already exists.${NC}"
...
echo -e "${BLUE}Creating Kind cluster with port mappings for ingress...${NC}"
