...
# Setup Kind cluster for ingress-nginx demo
set -e
...
echo -e "${GREEN}🚀 Setting up Kind cluster for ingress-nginx demo${NC}"
echo "================================="
...
if kind get clusters 2>/dev/null | grep -q "ingress-nginx-demo"; then
    echo -e "${YELLOW}⚠️  Cluster 'ingress-nginx-demo' already exists.${NC}"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        kubectl config use-context kind-ingress-nginx-demo
        exit 0
    fi
fi
