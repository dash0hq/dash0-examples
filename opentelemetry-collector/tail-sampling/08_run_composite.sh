...
echo "Starting OpenTelemetry Collector with config-composite.yaml"
echo "This configuration demonstrates the composite policy with rate allocation"
echo ""
...
docker run \
  ...
  --publish 4317:4317 \
  --publish 4318:4318 \
  -v "$(pwd)/config-composite.yaml:/etc/otelcol-contrib/config.yaml" \
