...
# Check if Contour is running
if ! kubectl get pods -n projectcontour -l app.kubernetes.io/name=contour 2>/dev/null | grep -q Running; then
    echo -e "${RED}❌ Contour is not running. Please run 03_install_contour.sh first.${NC}"
    exit 1
fi

echo -e "${BLUE}Step 1/4: Building Node.js application...${NC}"
"${SCRIPT_DIR}/05_build_nodejs_app.sh"

echo -e "${BLUE}Step 2/4: Deploying Node.js application...${NC}"
kubectl create namespace demo --dry-run=client -o yaml | kubectl apply -f -
kubectl apply -f "${PROJECT_ROOT}/services/nodejs-app/manifests/"

echo -e "${BLUE}Step 3/4: Deploying HTTPRoute resource...${NC}"
kubectl apply -f "${PROJECT_ROOT}/contour/httproute.yaml"
