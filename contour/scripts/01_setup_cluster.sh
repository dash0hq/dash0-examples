...
# Setup Kind cluster for Contour demo
set -e
...
echo -e "${GREEN}🚀 Setting up Kind cluster for Contour demo${NC}"
echo "================================="
...
if kind get clusters 2>/dev/null | grep -q "contour-demo"; then
    echo -e "${YELLOW}⚠️  Cluster 'contour-demo' already exists.${NC}"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        kubectl config use-context kind-contour-demo
        exit 0
    fi
fi
